...
__all__ = [
    ...
    "multiple_to_all_shortest_path_length",
    "predecessor",
]
...
def multiple_to_all_shortest_path_length(S, G, cutoff=None):
    """Computes the shortest path lengths between nodes in S to all nodes in `G`.

    Parameters
    ----------
    S : An iterable of source nodes
    G : NetworkX graph
    ...
    >>> G = nx.path_graph(5)
    >>> S = [0, 2, 3]
    >>> length = dict(nx.multiple_to_all_shortest_path_length(S, G))
    >>> for node in [0, 1, 2, 3, 4]:
    ...     print(f"1 - {node}: {length[1][node]}")
